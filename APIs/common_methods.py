    ...
    def socket_send_to_router(self, data):
        if hasattr(self, 'client'):
            ...
            self.client = my_socket.MyClient((self.config_file.get(
                "network", "host"), 5100), self.LOG, debug=True)

        if self.client.get_connected() or self.client.connect():
            if self.router_logged:
                pass
            else:
                # build msg
                self.common_para_dict["req_id"] = 123
                self.common_para_dict["pwd"] = common_APIs.get_md5(
                    self.config_file.get("app", "login_pwd"))
                login_msg = API_router_management.build_msg_password_login(
                    self.common_para_dict)
                ...
                if msg:
                    dst_package = self.get_package_by_keyword(
                        msg, ['um_login_pwd', 'success'])
                    if dst_package:
                        self.router_logged = True
                        self.config_file.set("app", "token", json.loads(
                            dst_package[0])['content']['result']['token'])
                        self.config_file.write(open(self.config_file_ori, "w"))
    ...
    def add_wifi_device(self, device_category_id, room_id=1):
        ...
        for i in range(5):
            ...
            msg = API_device_management.build_msg_add_device(
                common_para_dict, device_category_id=device_category_id)
