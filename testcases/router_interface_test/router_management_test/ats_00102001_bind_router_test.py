...
class Test(common_methods.CommMethod):
    def run(self):
        self.common_para_dict["pwd"] = get_md5(
            self.config_file.get("app", "login_pwd"))

        # 模拟APP登录操作，获取Token
        # um_login_pwd build msg
        msg = API_router_management.build_msg_password_login(
        ...
        msg_unbind = API_router_management.build_msg_unbind_router(
            self.common_para_dict)

        # dm_bind_router build msg
        msg_bind = API_router_management.build_msg_bind_router(
            self.common_para_dict, token=token)
