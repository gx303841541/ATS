...
class Test(common_methods.CommMethod):
    def run(self):
        ...
        if result and 'device_uuid' in result[1]:
            ...
            if self.socket_send_to_router(json.dumps(msg) + '\n'):
                ...
                if self.mysleep(65, feedback=add_success):
                    ...
                    return self.case_fail("Add device failed!")
            else:
        ...
        # build msg
        msg = API_room_homepage_management.build_msg_main_switch_offon(common_para_dict, offon='off')
        ...
        if data:
            dst_package = self.get_package_by_keyword(data, ['dm_set_total_control', 'result'], except_keyword_list=['mdp_msg'])
        ...
        # build msg
        msg = API_room_homepage_management.build_msg_main_switch_offon(common_para_dict, offon='on')
