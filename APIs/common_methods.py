	...
	def socket_send_to_router(self, data):
		...
		if self.client.is_connected() or self.client.connect():
			if self.router_logged:
				...
				login_msg = API_router_management.build_msg_password_login(self.common_para_dict)
				...
				if msg:
					...
					if dst_package:
						...
						self.config_file.set("app", "token", json.loads(dst_package[0])['content']['result']['token'])
						self.config_file.write(open(self.config_file_ori, "w"))
