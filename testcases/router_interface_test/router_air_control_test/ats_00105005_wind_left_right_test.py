...
class Test(common_methods.CommMethod):
    def run(self):
        ...
        # build msg
        login_msg = API_air_control.build_msg_wind_left_right(common_para_dict, 'off')
        ...
        # build msg
        login_msg = API_air_control.build_msg_wind_left_right(common_para_dict, 'on')
        ...
        # DB check
        result = self.get_router_db_info(['select * from TABLE_WIFI_DEVICE where device_uuid = "%s";' % common_para_dict["device_uuid"]])
        if 'device_uuid' in result[1]:
            if not re.search(r'"wind_left_right":"on"', result[1]['attribute']):
                return self.case_fail("wind on fail!")
        else:
