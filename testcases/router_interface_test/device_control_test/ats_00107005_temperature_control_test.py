...
class Test(common_methods.CommMethod):
    def run(self):
        ...
        # build msg
        temperature = random.randint(1, 65279)
        msg = API_device_control.build_msg_led_temperature_control(common_para_dict, temperature=temperature)
        ...
        if 'device_uuid' in result[1]:
            if not re.search(r'"temperature":%d' % (temperature), result[1]['attribute']):
