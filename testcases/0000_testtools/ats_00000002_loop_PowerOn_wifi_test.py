...
class Test(common_methods.CommMethod):
    def run(self):
        ...
        for i in range(1000):
            ...
            self.win_network.win_wifi_connect(self.config_file.get("network", "ssid_5G"))
            time.sleep(3)
            ret = self.win_network.win_ping_check(localip=self.win_network.win_wifi_get_wifi_service_ipv4(), remoteip=self.config_file.get("network", "host"))
            if ret == 'pass':
                self.LOG.debug('ping check passed on times: %s' % (i + 1))
            else:
                self.LOG.error('ping check failed on times: %s' % (i + 1))
                result = 1
            self.win_network.win_wifi_close()
            self.win_network.win_wifi_clean()
