...
class Test(common_methods.CommMethod):
    def run(self):
        ...
        # build msg
        rgb = {
            "r": random.randint(251, 251),
            "g": random.randint(0, 0),
            "b": random.randint(213, 213),
        }
        msg = API_device_control.build_msg_led_colour_control(common_para_dict, rgb=rgb)
        ...
        if 'device_uuid' in result[1]:
            if not re.search(r'"r":{0[r]},"g":{0[g]},"b":{0[b]}'.format(rgb), result[1]['attribute']):
